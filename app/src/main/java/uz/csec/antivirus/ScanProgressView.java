...
public class ScanProgressView extends View {
    ...
    public ScanProgressView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }
    ...
    private void init() {
        ...
        outerRingPaint.setColor(0xFF6AAEFF);
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        // Budget enough space from the edge so dots are OUTSIDE the outer ring
        float edgeBudget = outerStrokeWidth / 2f + dotsGap + dotRadiusLarge + safeEdge;

        // Inner progress ring radius (centerline)
        radius = halfMin - edgeBudget - gapBetweenRings - strokeWidth / 2f;
        if (radius < dp(24)) radius = dp(24); // avoid negative/too small

        // Outer ring (centerline)
        outerRadius = radius + strokeWidth / 2f + gapBetweenRings;

        // Dots radius (outside outer ring with visible gap)
        ...
        // Create a simple blue gradient for progress
    ...
    protected void onDraw(Canvas canvas) {
        ...
        // Draw static outer ring (no animation)
        ...
        // Draw dots around the perimeter
        drawDots(canvas);
    }

    private void drawDots(Canvas canvas) {
        // Draw dots at specific angles (top two large, bottom two small)
        float[] angles = {135, 225, 315, 45}; // degrees
    ...
    public void setProgress(float value) {
        ...
        // Update main text to show percentage
        invalidate();
    }
