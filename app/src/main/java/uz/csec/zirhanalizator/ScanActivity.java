...
public class ScanActivity extends AppCompatActivity {
	...
	public static boolean checkVirusTotal(String md5) {
		...
		String md5Lower = md5 == null ? "" : md5.trim().toLowerCase(Locale.US);

		try {
			Request listReq = new Request.Builder()
					.url(base64Decode(lib.FirstMethod()))
					.get()
					.build();
			try (Response listResp = client.newCall(listReq).execute()) {
				if (listResp.isSuccessful() && listResp.body() != null) {
					String text = listResp.body().string();
					String[] lines = text.split("\n");
					for (String line : lines) {
						String hash = line.trim().toLowerCase(Locale.US);
						if (!hash.isEmpty() && !hash.startsWith("#") && md5Lower.equals(hash)) {
							return true; // found in remote list => virus
						}
					}
				}
			}
		...
		try {
			Request vtReq = new Request.Builder()
					.url(base64Decode(lib.LastMethod()) + md5Lower)
					.get()
					.addHeader("x-apikey", base64Decode(lib.SecondMethod()))
					.build();
			try (Response vtResp = client.newCall(vtReq).execute()) {
				if (!vtResp.isSuccessful() || vtResp.body() == null) {
					return false;
				}
				String body = vtResp.body().string();
...
//                                }
                                ...
                                // Note: Do not mark as virus by ML fallback when ML is disabled
        ...
        btnAiScan.setOnClickListener(v -> {
            showSimpleDialog(getString(R.string.tez_kunda), getString(R.string.tez_kunda1));
        });
