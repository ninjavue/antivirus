...
public class CleanerActivity extends AppCompatActivity {
    ...
    private LinearLayout cleanerLayout, loadingContainer;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        insetsController.setAppearanceLightStatusBars(false); // Dark status bar for blue background
        ...
        cleanerLayout = findViewById(R.id.cleaner);
        ...
        cleanerLayout.setVisibility(View.GONE);
    ...
    private void loadCleanerData() {
        ...
        new LoadCleanerTask().execute();
    }

    // AsyncTask to load cleaner data
    private class LoadCleanerTask extends AsyncTask<Void, Void, CleanerData> {
        @Override
        ...
        @Override
        protected CleanerData doInBackground(Void... voids) {
            // Calculate sizes
            ...
            return new CleanerData(
                    new long[]{bigFiles, audioVideo, apks, junk},
                    new String[]{getString(R.string.big_files), getString(R.string.audio_and_video),
                            getString(R.string.apps_delete), getString(R.string.cash_memory)},
                    new boolean[]{true, true, true, true}
            );
        }

        @Override
        protected void onPostExecute(CleanerData data) {
            // Initialize data
            itemSizes = data.sizes;
            itemLabels = data.labels;
            itemSelected = data.selected;
            itemLayoutIds = new int[]{R.id.item1, R.id.item2, R.id.item3, R.id.item4};

            // Initialize cleaner items
            ...
            // Set clean button click listener
            tvCleanableDesc.setOnClickListener(v -> performCleanup());

            // Hide loading container and show main content
            loadingContainer.setVisibility(View.GONE);
            cleanerLayout.setVisibility(View.VISIBLE);
        }
    }

    // Data class to hold cleaner data
    private static class CleanerData {
        long[] sizes;
        String[] labels;
        boolean[] selected;

        CleanerData(long[] sizes, String[] labels, boolean[] selected) {
            this.sizes = sizes;
            this.labels = labels;
    ...
    private void initializeCleanerItems() {
        ...
        for (int i = 0; i < itemLayoutIds.length; i++) {
            ...
            if (itemView != null) {
                ...
                itemView.setOnClickListener(v -> toggleItemSelection(item));
            }
        }
    }
    ...
    private void updateAfterCleanup(long cleanedSize) {
        // Reset selection states to true
        for (int i = 0; i < itemSelected.length; i++) {
            itemSelected[i] = true;
        }

        // Update cleaner items
    ...
    private void performCleanup() {
        ...
        // Show loading UI during cleanup
        loadingContainer.setVisibility(View.VISIBLE);
        cleanerLayout.setVisibility(View.GONE);
    ...
    // AsyncTask to perform cleanup
    private class CleanupTask extends AsyncTask<Void, Void, Long> {
        @Override
        protected void onPreExecute() {
            // Show loading UI
            loadingContainer.setVisibility(View.VISIBLE);
        ...
        protected void onPostExecute(Long cleanedSize) {
            ...
            Toast.makeText(CleanerActivity.this, formatSize(cleanedSize) + " tozalandi", Toast.LENGTH_LONG).show();
        }
    }
